...
const Drawer = ({
  ...
  <>
    {isOpen && (
      <div className={cn('fixed inset-0 z-50', shouldScaleBackground && 'bg-black/80')}>
        <div
...
const DrawerTrigger = ({ onClick, children }: { onClick: () => void; children: React.ReactNode }) => (
  <button onClick={onClick}>{children}</button>
);

const DrawerClose = ({ onClick, children }: { onClick: () => void; children: React.ReactNode }) => (
  <button onClick={onClick}>{children}</button>
);
