...
export default async function uploadImage(buffer: Buffer): Promise<string> {
    try {
        ...
        const response: UploadResponse = await axios.post('https://uploader.nyxs.pw/upload', form, {
            headers: {
                ...form.getHeaders(),
            },
        });
        ...
        console.log('Uploaded to Nyxs successfully:', url);
 ...
 * @return {Promise<string>}
 */
async function uploadToPomf2(content: any) {
  console.log("Uploading to Pomf2...");
  ...
  try {
    ...
    console.error("Upload to Pomf2 failed:", err.message || err);
  }
}
 ...
 * @return {Promise<string>}
 */
async function uploadToTelegraph(buffer: any) {
  ...
  try {
    ...
    console.log("Uploaded to telegra.ph successfully!"+ img[0].src);
    return 'https://telegra.ph' + img[0].src;

  } catch (error) {
    const err = error as any;
    console.error("Upload to telegra.ph failed:", err.message || err);
  }
}
 ...
 * @return {Promise<string>}
 */
async function uploadToImgur(imageBuffer: { toString: (arg0: string) => any; }, clientId: any) {
  ...
  try {
    ...
    const err = error as any;
    console.error("Upload to Imgur failed:", err.message || err);
  }
}
 ...
 * @return {Promise<string>}
 */
async function uploadToFileIO(fileBuffer: any) {
  ...
  try {
    ...
    if (json.success) {
      console.log("Uploaded to File.io successfully!"+json.link);
      return json.link;
    } else {
    ...
    const err = error as any;
    console.error("Upload to File.io failed:", err.message || err);
  }
}
